...
export default function Cards({ countries, maxPages, setCountry, getCountries }) {
	...
	return countries.slice(0, maxPages).map((country, index) =>
		<div key={index + 105} tabIndex="0" className="countryCard bg-white dark:bg-darkblue hover:!scale-105 transition-all rounded-md overflow-hidden cursor-pointer shadow-md opacity-40 translate-y-6 scale-75 max-h-[25rem] max-w-[28rem] mx-auto" onClick={() => setCountry(country)}>
