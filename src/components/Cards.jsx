...
export default function Cards({ countries, maxPages, setCountry, getCountries }) {
	...
	return countries.slice(0, maxPages).map((country, index) =>
		<div key={index + 105} tabIndex="0" className="countryCard w-full bg-white dark:bg-darkblue hover:!scale-105 transition-all rounded-md overflow-hidden cursor-pointer shadow-md hover:shadow-lg opacity-40 translate-y-6 scale-75 max-h-[25rem] max-w-sm mx-auto" onClick={() => setCountry(country)}>
			...
			<section className="p-6 h-1/2">
				...
				<ul className="my-1">
					...
					<li><b>Capital:</b> {Array.isArray(country.capital) ? country.capital.join(", ") : country.capital || <i>No information</i>}</li>
				</ul>
			</section>
		</div>
	)
